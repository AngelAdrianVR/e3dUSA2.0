<template>
    <AppLayout title="Editar Solicitud de Muestra">
        <div class="px-4 sm:px-0">
            <div class="flex items-center space-x-2">
                <Back />
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                    Editar Solicitud de Muestra MUE-{{ sampleTracking.id.toString().padStart(4, '0') }}
                </h2>
            </div>
        </div>
...
export default {
    data() {
        const form = useForm({
            _method: 'PUT',
            name: this.sampleTracking.name,
            branch_id: this.sampleTracking.branch_id,
            contact_id: this.sampleTracking.contact_id,
            will_be_returned: this.sampleTracking.will_be_returned,
            expected_devolution_date: this.sampleTracking.expected_devolution_date,
            comments: this.sampleTracking.comments,
            items: this.sampleTracking.items.map(item => ({
                type: item.itemable_type.includes('NewProductProposal') ? 'new' : 'catalog',
                itemable_id: item.itemable_type.includes('Product') ? item.itemable_id : null,
                name: item.itemable_type.includes('NewProductProposal') ? item.itemable.name : '',
                description: item.itemable_type.includes('NewProductProposal') ? item.itemable.description : '',
    ...
    methods: {
        update() {
            this.form.post(route("sample-trackings.update", this.sampleTracking), {
                onSuccess: () => {
                    ElMessage.success('Solicitud de muestra actualizada');
                },
                onError: (errors) => {
                    console.log(errors);
        ...
        addItem(type) {
            if (type === 'catalog') {
                this.form.items.push({
                    type: 'catalog', itemable_id: null, quantity: 1, notes: '', product_data: null, loading: false,
                });
            } else {
                this.form.items.push({
                    type: 'new', name: '', description: '', quantity: 1, media: [],
                });
            }
        },
        removeItem(index) {
        ...
        async storeQuickBranch() {
            ...
            try {
                const response = await axios.post(route('sample-trackings.quick-store.branch'), this.quickBranchForm);
                if (response.status === 200) {
                    ...
                    this.quickBranchForm.name = ''; this.quickBranchForm.rfc = '';
                    ElMessage.success('Cliente/Prospecto creado exitosamente');
                }
            } catch (error) {
                if (error.response && error.response.status === 422) {
                    this.quickBranchForm.errors = Object.fromEntries(Object.entries(error.response.data.errors).map(([key, value]) => [key, value[0]]));
                } else {
                    console.error(error);
        ...
        async storeQuickContact() {
            ...
            try {
                const response = await axios.post(route('sample-trackings.quick-store.contact', { branch: this.form.branch_id }), this.quickContactForm);
                if (response.status === 200) {
                    const newContact = response.data;
                    this.availableContacts.push(newContact);
                    const parentBranch = this.localBranches.find(b => b.id === this.form.branch_id);
                    if (parentBranch) parentBranch.contacts.push(newContact);
                    this.form.contact_id = newContact.id;
                    this.contactModalVisible = false;
                    this.quickContactForm.name = ''; this.quickContactForm.charge = '';
                    ElMessage.success('Contacto creado exitosamente');
                }
            } catch (error) {
                if (error.response && error.response.status === 422) {
                    this.quickContactForm.errors = Object.fromEntries(Object.entries(error.response.data.errors).map(([key, value]) => [key, value[0]]));
                } else {
                    console.error(error);
    ...
    mounted() {
        ...
        // Cargar datos de productos para items existentes
