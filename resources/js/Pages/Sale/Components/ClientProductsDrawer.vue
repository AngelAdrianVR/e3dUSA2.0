<template>
    <!-- <el-drawer 
        ...
        <div class="p-4">
            ...
            <div v-else-if="!clientProducts.length">
                <p>Este cliente no tiene productos registrados.</p>
            </div>
            <div v-else class="space-y-3">
                <div v-for="product in clientProducts" :key="product.id" class="bg-gray-100 dark:bg-slate-800 p-3 rounded-lg">
                    <p class="font-semibold text-gray-800 dark:text-gray-200">{{ product.name }}</p>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Precio base: ${{ formatNumber(product.base_price) }}</p>
                </div>
            </div>
        </div>
    </el-drawer> -->
