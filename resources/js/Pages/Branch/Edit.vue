                ...
                <div class="bg-white dark:bg-slate-900 overflow-hidden shadow-xl sm:rounded-lg p-6 md:p-8">
                    ...
                    <form @submit.prevent="update">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-4">
                            <TextInput label="Nombre*" v-model="form.name" type="text" :error="form.errors.name" />
                            <TextInput label="RFC" v-model="form.rfc" type="text" :error="form.errors.rfc" />
                            <div class="md:col-span-2">
                                <TextInput label="Dirección" v-model="form.address" type="text" :error="form.errors.address" />
                            </div>
                            ...
                            <div>
                             ...
                             <TextInput label="¿Cómo nos conoció?" v-model="form.meet_way" type="text" :error="form.errors.meet_way" />
                        </div>
...
export default {
    data() {
        // Mapeamos los contactos para aplanar la estructura de 'details'
        const formattedContacts = this.branch.contacts.map(contact => {
            const phoneDetail = contact.details.find(d => d.type === 'Teléfono');
            const emailDetail = contact.details.find(d => d.type === 'Correo');
            return {
                id: contact.id,
                name: contact.name,
                charge: contact.charge,
                phone: phoneDetail ? phoneDetail.value : null,
                email: emailDetail ? emailDetail.value : null,
            };
        });
    ...
    props: {
        branch: Object,
        users: Array,
        branches: Array, // Renombrado para evitar conflicto con la prop 'branch'
    },
    methods: {
        update() {
            this.form.put(route("branches.update", this.branch.id), {
                onSuccess: () => {
