                    ...
                    <template v-for="(menu, index) in topMenus" :key="'top-' + index">
                        <SideNavLink class="relative" v-if="menu.show" :href="menu.route" :active="menu.active"
                            ...
                            <template #trigger>
                                ...
                                <!-- Borde animado: el usuario tiene una tarea EN PROCESO -->
                                <div v-if="menu.taskInProgress" class="task-glow absolute inset-0 rounded-xl pointer-events-none"></div>
                            ...
                            <template #tooltip>
                                <!-- Información de la tarea en proceso dentro del mismo tooltip del ítem -->
                                <div v-if="menu.taskInProgress" class="mt-1.5 pt-1.5 border-t border-white/20">
                                    <p class="text-[10px] font-bold text-amber-300 uppercase tracking-wide">Tarea en proceso</p>
                                    <p class="max-w-[230px] truncate" :title="menu.taskInProgress.title">{{ menu.taskInProgress.title }}</p>
                                    <p v-if="menu.taskInProgress.context" class="text-[10px] opacity-70 max-w-[230px] truncate">
                                        {{ menu.taskInProgress.context }}
                                    </p>
                                    <p v-if="menu.taskInProgress.count > 1" class="text-[10px] opacity-70">
                                        y {{ menu.taskInProgress.count - 1 }} más en proceso
                                    </p>
                                </div>
                            </template>
                            <template #content>
    ...
    data() {
        ...
        return {
            ...
            menus: [
                ...
                },
                {
                    ...
                    options: [
                        ...
                        },
                        {
                            ...
                            show: this.$page.props.auth.user.permissions?.includes('Ver tarifas'),
                            active: route().current('shipping-rates.*'),
                            notifications: false,
                        },
                    ],
                    dropdown: true,
                    show: this.$page.props.auth.user.permissions.includes('Ver envios')
                        || this.$page.props.auth.user.permissions?.includes('Ver tarifas')
                },
                {
                ...
                {
                    label: 'Proyectos',
                    icon: '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 12.75V12A2.25 2.25 0 0 1 4.5 9.75h15A2.25 2.25 0 0 1 21.75 12v.75m-8.69-6.44-2.12-2.12a1.5 1.5 0 0 0-1.061-.44H4.5A2.25 2.25 0 0 0 2.25 6v12a2.25 2.25 0 0 0 2.25 2.25h15A2.25 2.25 0 0 0 21.75 18V9a2.25 2.25 0 0 0-2.25-2.25h-5.379a1.5 1.5 0 0 1-1.06-.44Z" /></svg>',
    ...
    computed: {
        ...
        topMenus() {
            ...
            return this.menus.map(menu => {
                ...
                // Proyectos / Producción: indicador reactivo de tarea en proceso (borde animado + tooltip)
                if (menu.label === 'Proyectos') {
                    return { ...menu, taskInProgress: this.$page.props.active_project_task ?? null };
                }

                if (menu.label === 'Producción') {
                    return { ...menu, taskInProgress: this.$page.props.active_production_task ?? null };
                }
...
<style scoped>
/* Borde animado cuando el usuario tiene una tarea de proyecto EN PROCESO */
@keyframes task-glow {
    0%   { box-shadow: 0 0 0 2px rgba(245, 158, 11, .85), 0 0 12px 2px rgba(245, 158, 11, .55); }
    33%  { box-shadow: 0 0 0 2px rgba(59, 130, 246, .85), 0 0 12px 2px rgba(59, 130, 246, .55); }
    66%  { box-shadow: 0 0 0 2px rgba(16, 185, 129, .85), 0 0 12px 2px rgba(16, 185, 129, .55); }
    100% { box-shadow: 0 0 0 2px rgba(245, 158, 11, .85), 0 0 12px 2px rgba(245, 158, 11, .55); }
}

.task-glow {
    animation: task-glow 2.4s linear infinite;
}
